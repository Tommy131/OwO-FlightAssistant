...
- **自动更新检测（模块 `modules/update/`）**：启动后台查一次发行仓库，有新版本
  ...
  发行说明的渲染是**自己写的最小 Markdown 子集**（只认标题/列表/段落，
  先转义再包标签），没有接通用 Markdown 库 —— release body 是外部文本，
  通用渲染器会把 `<script>`、`<img onerror>` 原样吐出来。判定与渲染在
  `services/update-model.ts`（纯函数、38 条单测，其中一半是注入用例）。
