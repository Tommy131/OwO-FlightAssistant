...
| M6 | 纯计算单测（35 例）+ 架构/i18n 门禁 | 完成 |
| M7 | UI 与 store 测试、ESLint 门禁、超大文件拆分 | 待办 |
...
| §10 测试 | 纯计算部分已覆盖（Vitest，35 例）；**UI 与 store 仍无测试** | 组件测试要引 jsdom 与 testing-library，成本高于收益；先把最容易出错的几何与解析锁住 |
