...
| M6 | 纯计算、解析与规则引擎单测（180 例）+ 架构/i18n/ESLint 门禁 | 完成 |
...
| 差距 | 现状 | 为什么先不动 |
|---|---|---|
| §5.1 单文件 ~400 行 | 16 个文件超标，最大 `map-store.ts` 1344 行、`middleware-flight-data-adapter.ts` 800 行 | `map-canvas`（1467→709）、`map-page`（1026→143）已按职责拆开；`map-store` 抽出解析层与三个规则引擎（1726→1344）；适配器抽出遥测解析层（1047→800）。剩余的是**有状态的 IO 编排**（Zustand 副作用、WebSocket/轮询/健康监控），拆它们需要先有针对性的测试 |
| §10 测试 | 纯计算、响应解析、报文解码与解析工具已覆盖（Vitest，180 例）；**UI 与 store 编排仍无测试** | 组件测试要引 jsdom 与 testing-library，成本高于收益；先把最容易出错的几何与解析锁住 |
