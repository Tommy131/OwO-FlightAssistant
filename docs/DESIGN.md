...
| M6 | 单元测试补齐（几何计算与解析器优先）、ESLint 门禁 | 待办 |
...
| 差距 | 现状 | 为什么先不动 |
|---|---|---|
| §5.1 单文件 ~400 行 | 22 个文件超标，最大 `map-store.ts` 1726 行、`map-canvas.tsx` 1474 行 | 拆分是大改动，而目前**没有任何测试**保护。手册 §10 要求先有测试再重构，顺序不能反 |
| §10 测试 | 前端测试数为 **0**，无测试框架 | 优先补纯计算部分（`approach-beam` / `holding-geometry` / `papi-guidance` / `airport-outline`）—— 它们已被架构门禁保证是纯函数，最好测 |
...
`npm run check` 可一次跑完现有全部门禁（版本同步 / 架构 / i18n / 类型）。
...
- 地图模块 i18n 目前仅 zh/en 齐全，de 走回退链显示英文。
