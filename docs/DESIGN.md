...
| M6 | 纯计算、解析与规则引擎单测（157 例）+ 架构/i18n/ESLint 门禁 | 完成 |
...
| §10 测试 | 纯计算、响应解析、报文解码与解析工具已覆盖（Vitest，157 例）；**UI 与 store 编排仍无测试** | 组件测试要引 jsdom 与 testing-library，成本高于收益；先把最容易出错的几何与解析锁住 |
