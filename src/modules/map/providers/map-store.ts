...
function parseHoldingPattern(raw: unknown): MapHoldingPattern | null {
...
function parseRunwayNavaid(raw: unknown): MapRunwayNavaid | null {
  const map = toJsonMap(raw);
  if (!map) return null;
  const runway = pickString(map, ['runway'])?.toUpperCase();
...
function parseAerowayFeature(raw: unknown): MapAerowayFeature | undefined {
  const map = toJsonMap(raw);
...
function parseNearbyAirport(raw: unknown): MapAirportMarker | undefined {
  const map = toJsonMap(raw);
  if (!map) return undefined;
...
function parseTaxiwayFile(decoded: unknown): MapTaxiwayFileData | null {
 ...
 */
function parseRestrictedZone(raw: unknown): MapRestrictedZone | null {
  const map = toJsonMap(raw);
  if (!map) return null;
...
function parseAltitudeFt(raw: unknown): number | undefined {
